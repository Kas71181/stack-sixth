...
export default async function(req) {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      await base44.asServiceRole.entities.AcquisitionEvent.create({ organization_id: record.organization_id, owner_user_id: record.owner_user_id, event_name: "checkout_completed", properties: { plan: metadata.plan || record.plan, trial_days: 30 }, occurred_at: new Date().toISOString() });
