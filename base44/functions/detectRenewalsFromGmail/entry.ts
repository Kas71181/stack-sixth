...
export default async function(req) {
  try {
    ...
    const query = 'newer_than:2y (invoice OR receipt OR renewal OR subscription OR billing OR "payment received")';
    ...
    return Response.json({ connected: true, candidates: candidates.slice(0, 30), suggestions: candidates.filter((item) => item.renewal_date).slice(0, 20), scanned: Math.min(messages.length, 50) });
  } catch (error) {
