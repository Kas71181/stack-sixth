...
export default async function(req) {
  try {
    ...
    const params = new URLSearchParams({ mode: "subscription", "line_items[0][price]": priceId, "line_items[0][quantity]": "1", customer_email: user.email, payment_method_collection: "always", success_url: "https://stack-sixth-spend.base44.app/checkout/success?session_id={CHECKOUT_SESSION_ID}", cancel_url: "https://stack-sixth-spend.base44.app/checkout/cancelled", "metadata[base44_app_id]": secrets.get("BASE44_APP_ID"), "metadata[organization_id]": subscription.organization_id, "metadata[owner_user_id]": user.id, "metadata[plan]": normalizedPlan, "metadata[billing_interval]": interval, "subscription_data[trial_period_days]": "30", "subscription_data[metadata][base44_app_id]": secrets.get("BASE44_APP_ID"), "subscription_data[metadata][organization_id]": subscription.organization_id, "subscription_data[metadata][owner_user_id]": user.id, "subscription_data[metadata][plan]": normalizedPlan, "subscription_data[metadata][billing_interval]": interval });
