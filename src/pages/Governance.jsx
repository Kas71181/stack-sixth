...
export default function Governance() {
  ...
  return <div className="space-y-6">
    ...
    <div className="tab-track grid grid-cols-2 gap-1 sm:flex sm:overflow-x-auto">{TABS.map(({ key, label, icon: Icon }) => <button key={key} onClick={() => selectTab(key)} className={`flex items-center justify-start gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition-all sm:shrink-0 sm:justify-center ${tab === key ? "bg-card text-primary shadow-sm ring-1 ring-border" : "text-muted-foreground hover:bg-card/60 hover:text-foreground"}`}><Icon className="h-4 w-4" />{label}</button>)}</div>
