...
export default function Governance() {
  ...
  const [tab, setTab] = useState(TABS.some((item) => item.key === requested) ? requested : "overview");
  const selectTab = (key) => {
    setTab(key);
    window.history.replaceState({}, "", `/governance${key === "overview" ? "" : `?tab=${key}`}`);
  };
