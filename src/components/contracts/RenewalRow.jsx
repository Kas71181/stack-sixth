...
export default function RenewalRow({ contract, onOpen, discount }) {
  const autoRenewal = contract.auto_renewal_status || (contract.auto_renews === true ? "yes" : contract.auto_renews === false ? "no" : "unknown");
  ...
  return <button onClick={() => onOpen(contract)} className="group grid w-full gap-4 border-b border-border px-4 py-4 text-left transition-colors last:border-b-0 hover:bg-muted/40 sm:grid-cols-[minmax(180px,1.4fr)_1fr_.8fr_.8fr_auto] sm:items-center">
    <div className="flex min-w-0 items-center gap-3"><ToolLogo name={contract.vendor_name} /><div className="min-w-0"><p className="truncate text-sm font-bold">{contract.vendor_name}</p><p className="truncate text-xs text-muted-foreground">{contract.contract_name || contract.contract_type || "Software subscription"}</p>{discount && <div className="mt-1"><DiscountOffer offer={discount} compact /></div>}</div></div>
    ...
    <div><p className="text-xs font-semibold capitalize">{autoRenewal}</p><p className="text-[11px] text-muted-foreground"><RotateCw className="mr-1 inline h-3 w-3" />Auto-renew</p></div>
