...
export default function RenewalList({ contracts, onUpdated, offersByTool = {} }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
    ...
    {contracts.map((contract) => <RenewalRow key={contract.id} contract={contract} onOpen={setSelected} discount={offersByTool[contract.vendor_name]?.[0]} />)}
    {!contracts.length && <p className="px-5 py-12 text-center text-sm text-muted-foreground">No renewals match these filters.</p>}
    <RenewalDetailSheet contract={selected} discount={selected ? offersByTool[selected.vendor_name]?.[0] : null} open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)} onUpdated={() => { onUpdated(); setSelected(null); }} />
  </section>;
}