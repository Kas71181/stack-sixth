...
export default function RenewalDetailSheet({ contract, open, onOpenChange, onUpdated, discount }) {
  ...
  const deadline = contract.notice_deadline ? new Date(`${contract.notice_deadline}T12:00:00`) : renewal && contract.notice_period_days != null ? subDays(renewal, contract.notice_period_days) : null;
  ...
  const fields = [
    ...
    [RotateCw, "Auto-renewal", autoRenewal],
    ...
    [CalendarClock, "Notice deadline", deadline ? format(deadline, "MMMM d, yyyy") : "Not established"],
    [Database, "Source", contract.renewal_source_label || contract.renewal_source || "Unknown"],
    [CalendarClock, "Last verified", contract.last_verified_at ? format(new Date(contract.last_verified_at), "MMM d, yyyy") : "Not verified"],
  ];
