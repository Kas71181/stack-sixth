...
export default function PartnerCodeField({ onApply, applied, loading, error }) {
  ...
  return <div className="rounded-xl border border-border/60 bg-muted/30 p-4">
    ...
    {applied && <div className="mt-3 flex gap-2 rounded-xl bg-emerald-50 p-3 text-emerald-800"><CheckCircle2 className="h-4 w-4 shrink-0" /><div><p className="text-sm font-bold">{applied.partner_name} Partner Benefit Applied</p><p className="mt-0.5 text-xs">You've received {applied.benefit_duration_months} months of Stack Sixth {applied.eligible_plan.toLowerCase()} at no cost, courtesy of {applied.campaign_name}.</p><p className="mt-2 text-xs"><s>$199/month</s> · <strong>$0 for 6 months</strong> · $199/month afterward only with payment consent.</p></div></div>}
  </div>;
}