...
export default function PricingPlanCard({ plan, interval, selected, recommended, loading, onSelect }) {
  ...
  return <article className={`relative flex h-full flex-col rounded-2xl border p-5 transition-all ${selected ? "border-primary bg-primary/5 shadow-glow-sm" : "border-border/70 bg-card/80 shadow-sm"} ${plan.popular ? "ring-2 ring-primary/20" : ""}`}>
    ...
    <button onClick={() => onSelect(plan)} disabled={loading} className={`w-full rounded-xl px-4 py-2.5 text-sm font-bold active:scale-[0.96] disabled:cursor-wait disabled:opacity-60 ${plan.popular ? "bg-primary text-primary-foreground" : "border border-border bg-background"}`}>{loading ? "Opening checkout..." : plan.cta || (enterprise ? "Contact Sales" : `Start ${plan.name} Trial`)}</button>
  </article>;
}