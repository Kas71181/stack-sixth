...
export default function PricingPlanCard({ plan, interval, selected, recommended, onSelect }) {
  ...
  const free = plan.plan_key === "FREE_LAUNCH";
  ...
  return <article className={`relative flex h-full flex-col rounded-2xl border p-5 transition-all ${selected ? "border-primary bg-primary/5 shadow-glow-sm" : "border-border/70 bg-card/80 shadow-sm"} ${plan.popular ? "ring-2 ring-primary/20" : ""}`}>
    ...
    <h2 className="text-xl font-extrabold">{plan.name}</h2><p className="mt-1 min-h-10 text-xs text-muted-foreground">{plan.target}</p>
    <div className="my-5">{enterprise ? <p className="text-3xl font-black">Custom</p> : <><p className="text-3xl font-black">${price?.toLocaleString()}<span className="text-sm font-medium text-muted-foreground">/{free ? "90 days" : interval === "annual" ? "year" : "month"}</span></p>{free ? <p className="mt-1 text-xs font-semibold text-emerald-600">No credit card required</p> : interval === "annual" && <p className="mt-1 text-xs font-semibold text-emerald-600">Save ${annualSave.toLocaleString()} annually</p>}</>}</div>
    ...
    <button onClick={() => onSelect(plan)} className={`w-full rounded-xl px-4 py-2.5 text-sm font-bold active:scale-[0.96] ${free || plan.popular ? "bg-primary text-primary-foreground" : "border border-border bg-background"}`}>{plan.cta || (enterprise ? "Contact Stack Sixth" : `Choose ${plan.name}`)}</button>
  </article>;
}