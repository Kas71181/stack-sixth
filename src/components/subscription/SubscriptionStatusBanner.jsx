...
export default function SubscriptionStatusBanner({ enabled }) {
  ...
  return <div className="border-b border-primary/15 bg-primary/5 px-4 py-2 text-center text-xs text-foreground"><strong>{subscription.subscription_status === "PROMOTIONAL" ? `${subscription.promotional_partner} Partner Access` : data.plan_status === "legacy" ? "Legacy Free Launch" : "Free Launch"}</strong>{data.days_remaining !== null && ` · ${data.days_remaining} days remaining`}<Link to="/pricing" className="ml-2 font-bold text-primary hover:underline">Choose My Plan</Link></div>;
}